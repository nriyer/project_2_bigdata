...
#use training and test 50-50 split, normalized using 'range'

hc.complete1=hclust(dist(train1), method="complete")
hc.average1=hclust(dist(train1), method="average")
hc.single1=hclust(dist(train1), method="single")
plot(hc.complete1)
...
#use training and test 60-40 split, normalized using 'range'
...
plot(hc.single2, main="Single Linkage", xlab="", sub="", cex=.9)
dev.off()
...
#Ward Hierarchical Clustering (only for 70-30 train/test since it is an additional method to the three required)
...
#training and test for knn (60-40 train-test)
...
#training and test for knn (70-30 train-test)
